...
public class EmployerController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        // Vérifier si l'utilisateur est autorisé à accéder à cette interface
        employe emp = session.getEmploye();
        if (emp == null || emp.getRole().equalsIgnoreCase("employe")) {
            afficherErreur("Accès refusé", "Vous n'avez pas les permissions pour accéder à cette interface.");
            return;
        }
    ...
    private void ajouterEmploye() {
        ...
        try {
            ...
            nv.setRole(comboRoleAjout.getValue());
            nv.setDate_embauche(dateEmbaucheAjout.getValue());
            nv.setIdEntreprise(idEntrepriseConnecte);

            int idEmploye = employeCRUD.add(nv);

            afficherSucces("Succès", "L'employé a été ajouté avec l'ID: " + idEmploye);
            resetFormulaireAjout();
            retourListe();
            ...
            afficherErreur("Erreur", "Impossible d'ajouter l'employé : " + e.getMessage());
        } catch (NumberFormatException e) {
            marquerErreur(txtTelephoneAjout);
            lblErrorAjout.setText("Format téléphone invalide.");
            afficherErreur("Erreur", "Le téléphone doit être un nombre valide.");
        } catch (Exception e) {
            System.err.println("Erreur inattendue : " + e.getMessage());
            e.printStackTrace();
            lblErrorAjout.setText("Erreur inattendue : " + e.getMessage());
    ...
    private void sauvegarderModifications() {
        ...
        try {
            ...
            employeSelectionne.setRole(comboRole.getValue());
    ...
    private VBox creerCarteEmploye(employe emp) {
        ...
        if (emp.getRole().equals("administrateur entreprise")) {
    ...
    private Label creerBadgeRole(String roleStr) {
        Label l = new Label(roleStr);
        String s = "-fx-font-size: 10px; -fx-font-weight: bold; -fx-padding: 3 8; -fx-background-radius: 10;";
        if (roleStr.equals("administrateur entreprise")) {
            l.setStyle(s + "-fx-background-color: #eff6ff; -fx-text-fill: #2563eb;");
        } else if (roleStr.equals("rh")) {
            l.setStyle(s + "-fx-background-color: #fdf2f8; -fx-text-fill: #db2777;");
        } else if (roleStr.equals("chef projet")) {
            l.setStyle(s + "-fx-background-color: #f0fdf4; -fx-text-fill: #16A34A;");
        } else {
            l.setStyle(s + "-fx-background-color: #f1f5f9; -fx-text-fill: #475569;");
        }
        return l;
    }
    ...
    private void remplirFormulaireDetails(employe emp) {
        ...
        txtPoste.setText(emp.getPoste()); comboRole.setValue(emp.getRole());
        dateEmbauche.setValue(emp.getDate_embauche());
        lblTitreDetails.setText(emp.getPrenom() + " " + emp.getNom());
        lblError.setText("");
        comboRole.setDisable(emp.getRole().equals("administrateur entreprise"));
    ...
    private void initComboBoxRoles() {
        List<String> roles = List.of("rh", "employe", "chef projet", "administrateur entreprise");
    ...
    private String parseRole(String t) {
        if(t==null) return "employe";
        String v = t.trim().toLowerCase();
        if(v.contains("rh")) return "rh";
        if(v.contains("chef")) return "chef projet";
        if(v.contains("admin")) return "administrateur entreprise";
        return "employe";
    }
